...
public class DayTimeEvent extends BukkitRunnable {
    public int id;
    public DayTimeEvent(int _id){
        id=_id;
    }
    @Override
    public void run() {
        ...
        while(onlinePlayers.hasNext()){
            String command = Reader.config.getString("dayTimeEvent."+id+".command");
            String message = Reader.config.getString("dayTimeEvent."+id+".message");
            Player player = (Player) onlinePlayers.next();
            int dayTime = Reader.data.getInt("dayTime."+player.getName());
            int time = Reader.config.getInt("dayTimeEvent."+id+".time");
            if(dayTime==time) {
